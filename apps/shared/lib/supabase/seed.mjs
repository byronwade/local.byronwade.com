	...
	static generateEnhancedFakeBusinesses(city, category, limit) {
		...
		for (let i = 0; i < limit; i++) {
			...
			businesses.push({
				...
				photos: Array.from({ length: faker.number.int({ min: 1, max: 5 }) }, (_, idx) => `https://picsum.photos/800/600?random=${businessName.replace(/\s+/g, "")}_${idx}`),
			...
			try {
				...
				for (const realBusiness of realBusinessData) {
					...
					const business = {
						...
						photos: realBusiness.photos.length > 0 ? realBusiness.photos : [`https://picsum.photos/800/600?random=${realBusiness.name.replace(/\s+/g, "")}_main`, `https://picsum.photos/800/600?random=${realBusiness.name.replace(/\s+/g, "")}_interior`],
	...
	for (const business of seededData.businesses.slice(0, 50)) {
		...
		for (let i = 0; i < photoCount; i++) {
			photos.push({
				id: faker.string.uuid(),
				business_id: business.id,
				url: `https://picsum.photos/800/600?random=${business.id.replace(/\D/g, "").slice(-6)}${i}`,
...
async function seedReviews() {
	...
	while (reviews.length < SEED_CONFIG.reviews && attempts < maxAttempts) {
		...
		reviews.push({
			...
			photos: randomBoolean(0.3) ? [`https://picsum.photos/400/300?random=${reviews.length + 1000}`] : [],
...
async function seedCompanies() {
	...
	for (let i = 0; i < SEED_CONFIG.companies; i++) {
		...
		companies.push({
			...
			logo_url: `https://picsum.photos/200/200?random=logo${i}`,
			banner_url: `https://picsum.photos/1200/400?random=banner${i}`,
...
async function seedBlogPosts() {
	...
	for (let i = 0; i < SEED_CONFIG.blogPosts; i++) {
		...
		posts.push({
			...
			featured_image: `https://picsum.photos/1200/600?random=${posts.length + 2000}`,
			author_id: author.id,
...
async function seedEvents() {
	...
	for (let i = 0; i < SEED_CONFIG.events; i++) {
		...
		events.push({
			...
			featured_image: `https://picsum.photos/1200/600?random=${events.length + 3000}`,
			gallery: Array.from({ length: 3 }, (_, idx) => `https://picsum.photos/800/600?random=${events.length + 3100 + idx}`),
...
async function seedAdditionalTables() {
	...
	for (let i = 0; i < SEED_CONFIG.courses; i++) {
		...
		courses.push({
			...
			category_id: randomElement(seededData.categories)?.id || null,
			featured_image: `https://picsum.photos/800/600?random=${courses.length + 4000}`,
