...
export const VirtualBusinessList = ({ businesses = [], onLoadMore, loading = false, hasMore = true, containerHeight = 600, onBusinessClick, className = "" }) => {
	const renderBusinessCard = useCallback(
		(business, index) => {
			return (
				<div className="business-card p-4 border-b border-gray-200 hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => onBusinessClick?.(business)} data-business-id={business.id} data-preload={`/biz/${business.id}`}>
			...
			return (
				<div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${columns}, 1fr)` }}>
					{row.map((business, colIndex) => (
						<div key={business.id} className="business-grid-card bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer" onClick={() => onBusinessClick?.(business)} data-business-id={business.id} data-preload={`/biz/${business.id}`}>
							{/* Business Image */}
