...
export class EnterpriseSEOManager {
	constructor(options = {}) {
		this.siteConfig = {
			name: options.siteName || "Local ByteRover Directory",
			url: options.baseUrl || "https://local.byronwade.com",
			description: options.siteDescription || "Discover local businesses, events, and community resources in your area",
			logo: options.logo || "https://local.byronwade.com/logos/ThorbisLogo.webp",
	...
	async generatePageMetadata(pageConfig) {
		...
		try {
			...
			const structuredData = await this.generateStructuredData(pageConfig);
			...
			logger.error("Failed to generate page metadata:", error);
