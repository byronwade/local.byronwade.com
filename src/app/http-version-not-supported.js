		...
		<div className="flex items-center justify-center min-h-screen bg-background text-foreground p-4">
			<div className="max-w-md text-center">
				<div className="relative inline-block mb-8">
					...
					<p className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-background text-sm font-medium px-2 bg-red-600 rounded-full">Version Not Supported</p>
				</div>
				...
				<div className="flex gap-4 justify-center">
					<Button className="bg-red-600 hover:bg-red-700" onClick={handleRefresh}>
				...
				<div className="mt-12 text-sm text-muted-foreground/80">
					...
					<div className="flex justify-center gap-6 mt-4">
						<Link href="/support" className="flex items-center gap-1.5 hover:text-red-600 transition-colors">
							<HelpCircle className="w-4 h-4" /> Get Help
						</Link>
						<Link href="/contact-support" className="flex items-center gap-1.5 hover:text-red-600 transition-colors">
							<Settings className="w-4 h-4" /> Technical Support
						</Link>
					</div>
					<div className="mt-6 text-xs text-muted-foreground/60">
						Error Code: <span className="font-mono text-red-600">HTTP_505_{Date.now().toString(36).toUpperCase()}</span>
					</div>
				</div>
			</div>
		</div>
