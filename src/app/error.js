		...
		<div className="flex items-center justify-center min-h-screen bg-background text-foreground p-4">
			<div className="max-w-md text-center">
				<div className="relative inline-block mb-8">
					...
					<p className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-background text-sm font-medium px-2 bg-red-600 rounded-full">Server Error</p>
				</div>
				...
				{isDevMode && error && (
					<div className="mt-12 text-left text-sm bg-muted/40 border rounded-md p-4">
						<p className="font-semibold text-red-600 mb-2">Technical Details</p>
						<pre className="whitespace-pre-wrap text-muted-foreground">{error.message || "No error message provided."}{error.stack && `\n\n${error.stack}`}</pre>
					</div>
				)}

				<div className="mt-6 text-xs text-muted-foreground/60">Error Code: <span className="font-mono text-red-600">ERR_{Date.now().toString(36).toUpperCase()}</span></div>
			</div>
		</div>
	);
}
