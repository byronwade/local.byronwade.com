 ...
 */
export default function AccountingModule({ industryType, customizations = {}, pathname, params }) {
	...
	return (
		<div className="container mx-auto p-6 space-y-6">
									...
									{accountingData.recentTransactions.map((transaction) => (
										<div key={transaction.id} className="flex items-center justify-between p-3 border rounded-lg">
											<div className="flex items-center space-x-3">
												<div className={cn("h-8 w-8 rounded-full flex items-center justify-center", transaction.type === "income" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600")}>{transaction.type === "income" ? <TrendingUp className="h-4 w-4" /> : <BarChart3 className="h-4 w-4" />}</div>
												<div>
								...
								{accountingData.recentTransactions.map((transaction) => (
									<div key={transaction.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
										<div className="flex items-center space-x-4">
											<div className={cn("h-10 w-10 rounded-full flex items-center justify-center", transaction.type === "income" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600")}>{transaction.type === "income" ? <TrendingUp className="h-5 w-5" /> : <BarChart3 className="h-5 w-5" />}</div>
											<div>
							...
							<div className="space-y-3">
								{accountingData.accountsReceivable.map((receivable) => (
									<div key={receivable.id} className="flex items-center justify-between p-4 border rounded-lg">
										<div className="flex items-center space-x-4">
											<div className={cn("h-10 w-10 rounded-full flex items-center justify-center", receivable.status === "overdue" ? "bg-red-100 text-red-600" : receivable.status === "due" ? "bg-orange-100 text-orange-600" : "bg-blue-100 text-blue-600")}>{receivable.status === "overdue" ? <AlertCircle className="h-5 w-5" /> : receivable.status === "due" ? <Clock className="h-5 w-5" /> : <CheckCircle className="h-5 w-5" />}</div>
											<div>
			...
			{customizations.enableJobProfitAnalysis && (
				<Card>
					<CardHeader>
						<CardTitle>Job Profit Analysis</CardTitle>
						<CardDescription>Industry-specific profit tracking by job</CardDescription>
					</CardHeader>
					<CardContent>
						<p className="text-muted-foreground">Job-specific profit analysis will be displayed here.</p>
					</CardContent>
				</Card>
			)}
		</div>
