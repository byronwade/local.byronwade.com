...
export default function FeaturedBusinessesSection({ section, onInteraction, isPersonalized = false, personalizationScore = 0, userId, sessionId, className = "", ...props }) {
	...
	const renderRating = (rating, reviewCount) => (
		<div className="flex items-center gap-1">
			<div className="flex">
				{[1, 2, 3, 4, 5].map((star) => (
					<Star key={star} className={cn("w-4 h-4", star <= rating ? "text-yellow-400 fill-current" : "text-gray-300")} />
				))}
			</div>
	...
	const renderBusinessCardGrid = (business, index) => (
		<motion.div key={business.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} whileHover={{ y: -5, scale: 1.02 }} onHoverStart={() => setHoveredBusiness(business.id)} onHoverEnd={() => setHoveredBusiness(null)} onClick={() => handleBusinessClick(business)} className={cn("bg-white rounded-2xl shadow-lg hover:shadow-xl", "transition-all duration-300 cursor-pointer", "overflow-hidden group relative")}>
			...
			<div className="relative h-48 bg-gray-200">
				{business.primary_image ? (
					...
					<div className="w-full h-full bg-gradient-to-br from-gray-100 to-gray-300 flex items-center justify-center">
						<span className="text-4xl text-gray-400">{business.name?.charAt(0)}</span>
					</div>
				)}

				{/* Favorite button */}
				<button onClick={(e) => handleFavoriteToggle(e, business)} className={cn("absolute top-3 right-3 p-2 rounded-full", "bg-white/90 backdrop-blur-sm shadow-lg", "transition-all duration-300", "hover:scale-110", favorites.has(business.id) ? "text-red-500" : "text-gray-600")}>
					<Heart className={cn("w-5 h-5", favorites.has(business.id) && "fill-current")} />
				</button>

				{/* Personalization indicator */}
				{isPersonalized && personalizationScore > 0.5 && <div className="absolute top-3 left-3 bg-blue-500 text-white px-2 py-1 rounded-full text-xs font-medium">Recommended</div>}
			</div>
			...
			<div className="p-6">
				...
				<div className="mb-3">
					<h3 className="text-xl font-semibold text-gray-900 mb-1 group-hover:text-blue-600 transition-colors">{business.name}</h3>
					{business.business_categories?.[0]?.category && <p className="text-sm text-blue-600 font-medium">{business.business_categories[0].category.name}</p>}
				</div>
				...
				{/* Description */}
				{business.description && <p className="text-gray-600 text-sm mb-4 line-clamp-2">{business.description}</p>}
				...
				<div className="flex items-center justify-between">
					<div className="flex gap-2">
						{business.phone && (
							<button onClick={(e) => handleQuickAction(e, "call", business)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Call">
								<Phone className="w-4 h-4" />
							</button>
						)}
						<button onClick={(e) => handleQuickAction(e, "directions", business)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Directions">
							<MapPin className="w-4 h-4" />
						</button>
						{business.website && (
							<button onClick={(e) => handleQuickAction(e, "website", business)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Website">
								<ExternalLink className="w-4 h-4" />
							</button>
						)}
						<button onClick={(e) => handleQuickAction(e, "share", business)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Share">
		...
		<motion.div key={business.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} onClick={() => handleBusinessClick(business)} className={cn("bg-white rounded-xl shadow-md hover:shadow-lg", "transition-all duration-300 cursor-pointer", "overflow-hidden flex p-4 gap-4")}>
			...
			<div className="w-24 h-24 bg-gray-200 rounded-lg flex-shrink-0">
				{business.primary_image ? (
					...
					<div className="w-full h-full bg-gradient-to-br from-gray-100 to-gray-300 rounded-lg flex items-center justify-center">
						<span className="text-xl text-gray-400">{business.name?.charAt(0)}</span>
					</div>
				)}
			</div>
			...
			<div className="flex-1 min-w-0">
				<div className="flex items-start justify-between mb-2">
					...
					<button onClick={(e) => handleFavoriteToggle(e, business)} className={cn("p-1 rounded-full transition-colors", favorites.has(business.id) ? "text-red-500" : "text-gray-400")}>
						<Heart className={cn("w-5 h-5", favorites.has(business.id) && "fill-current")} />
					</button>
				</div>
				...
				{/* Address */}
				{business.address && <p className="text-gray-500 text-sm truncate mb-2">{business.address}</p>}

				{/* Quick actions */}
				<div className="flex gap-2">
					{business.phone && (
						<button onClick={(e) => handleQuickAction(e, "call", business)} className="text-xs bg-blue-50 text-blue-600 px-3 py-1 rounded-full hover:bg-blue-100 transition-colors">
							Call
						</button>
					)}
					<button onClick={(e) => handleQuickAction(e, "directions", business)} className="text-xs bg-gray-50 text-gray-600 px-3 py-1 rounded-full hover:bg-gray-100 transition-colors">
						Directions
					</button>
				</div>
			</div>
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				...
				<div className="flex justify-center mb-8">
					<div className="bg-white rounded-lg p-1 shadow-sm">
						<button onClick={() => setViewMode("grid")} className={cn("px-4 py-2 rounded-md text-sm font-medium transition-colors", viewMode === "grid" ? "bg-blue-500 text-white" : "text-gray-500 hover:text-gray-700")}>
							Grid View
						</button>
						<button onClick={() => setViewMode("list")} className={cn("px-4 py-2 rounded-md text-sm font-medium transition-colors", viewMode === "list" ? "bg-blue-500 text-white" : "text-gray-500 hover:text-gray-700")}>
							List View
						</button>
					</div>
				</div>
