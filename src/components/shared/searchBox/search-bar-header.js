...
import { ArrowRight, Search, X, Clock, Mic, MicOff, Send, Plus, Image, FileText, Camera, Upload } from "react-feather";
...
const SearchBarOnly = () => {
	...
	return (
		<div className="relative w-full" ref={dropdownRef}>
			<form className={`relative z-10 flex flex-col w-full h-full min-w-0 p-2 ml-6 border rounded-lg shadow-sm transition-all duration-200 ${aiMode ? "bg-blue-50 dark:bg-blue-950/30 border-blue-400 dark:border-blue-500" : "bg-background border-border"} ${isDraggingOver ? "border-dashed border-blue-500 bg-blue-100 dark:bg-blue-900/20 shadow-blue-500/20 shadow-xl scale-105" : "focus-within:border-blue-500 dark:focus-within:border-blue-400"}`} onSubmit={handleFormSubmit} {...dragHandlers}>
					...
					<div className="relative flex items-center space-x-2 ml-2">
						...
						{!aiMode && (
							<div onDragOver={(e) => e.preventDefault()} onDrop={(e) => e.preventDefault()}>
								<LocationDropdown size="small" className="text-sm h-6" />
							</div>
						)}
						<Button
							size="icon"
