...
const useSearchStore = create((set, get) => ({
	...
	fetchCityAndStateFromCoordinates: async (lat, lng) => {
		try {
			...
			// Return the data with fallback values
			return {
				city: data.city || "Unknown",
				state: data.state || "Location",
				formatted_address: data.formatted_address || `${data.city || "Unknown"}, ${data.state || "Location"}`,
			};
		} catch (error) {
			console.error("Error reverse geocoding:", error);
			console.error("Input coordinates:", { lat, lng });
			throw error;
		}
	},
}));
