				...
				{steps.map((stepItem, index) => (
					<div key={stepItem.id} className="flex items-center flex-1">
						<div className="flex flex-col items-center">
							<div className={`flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all duration-300 ${currentStep > stepItem.id ? "bg-primary border-primary text-primary-foreground shadow-lg" : currentStep === stepItem.id ? "bg-primary/10 border-primary text-primary shadow-md" : "border-muted-foreground/30 text-muted-foreground/50"}`}>
								{currentStep > stepItem.id ? <CheckCircle className="w-5 h-5" /> : React.createElement(stepItem.icon, { className: "w-5 h-5" })}
							</div>
