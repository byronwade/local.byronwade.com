...
export async function generateMetadata({ params }) {
	...
	try {
		const { data: business, error } = await BusinessDataFetchers.getBusinessProfile(resolvedParams.id);
		...
		// If we reach here, business data was successfully retrieved
		...
		const rating = business.rating ? `${business.rating}/5 stars` : "";
...
async function getBusinessData(businessId) {
	...
	try {
		const { data: business, error } = await BusinessDataFetchers.getBusinessProfile(businessId);
