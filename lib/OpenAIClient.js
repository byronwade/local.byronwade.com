...
async function getBusinessInfo(businessName) {
	try {
		console.log("Mock fetching business info for:", businessName);

		const { hits } = await algoliaIndex.search(businessName, {
			hitsPerPage: 5, // Fetch up to 5 potential matches
		});

		console.log("Mock received data from Algolia:", hits);

		return hits.length > 0 ? hits : null;
	} catch (error) {
		console.error("Mock error fetching business information:", error);
		return null;
	}
}
