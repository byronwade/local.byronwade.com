...
// Generate business photos (reliable placeholder URLs)
const generateBusinessPhotos = (businessId, category = 'business') => {
	const photoCount = faker.number.int({ min: 3, max: 8 });
	return Array.from({ length: photoCount }, (_, i) => {
		// Use Picsum with business ID for consistency
		const seed = businessId ? `${businessId}-${i}` : faker.number.int({ min: 1, max: 10000 });
