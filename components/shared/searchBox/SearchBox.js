...
import { ChevronDown, ChevronRight, X, Search, ArrowLeft, ArrowRight } from "react-feather";
...
const SearchBox = ({ showFilterButton, showSortButton, showLocationButton, showSparklesButton }) => {
				...
				<form className="relative z-10 w-full h-full min-w-0 p-3 bg-gray-900" onSubmit={handleSubmit}>
					...
					<div className="relative flex flex-col items-center flex-1 w-full gap-4 transition-all duration-300">
						<div className="relative flex self-start justify-between flex-1 w-full min-w-0 pb-2 border-b">
							...
							/>
							{searchQuery && (
								<button type="submit" className="absolute top-0 right-0 h-full px-2 py-1 text-sm font-medium text-white bg-blue-600 rounded-md focus:outline-none">
									<Search className="w-4 h-4" />
								</button>
							)}
						</div>
...
// Full Search Box
export const FullSearchBox = () => <SearchBox showFilterButton={true} showSortButton={true} showLocationButton={true} showSparklesButton={true} />;

// Search Bar Only
export const SearchBarOnly = () => <SearchBox showFilterButton={true} showSortButton={false} showLocationButton={false} showSparklesButton={true} />;
