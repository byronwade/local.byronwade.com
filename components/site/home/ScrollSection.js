		...
		<div className="w-full">
			<div className="relative w-full overflow-hidden group">
				...
				<div className="relative">
					<div ref={(el) => (scrollContainerRefs.current[title] = el)} onScroll={() => handleScroll(title)} className="flex flex-row gap-8 px-6 py-8 overflow-x-auto sm:px-12 lg:px-24 scrollbar-hide scroll-smooth" style={{ scrollBehavior: "smooth" }}>
					...
					{canScrollLeft[title] && (
						<div className="absolute inset-y-0 left-0 items-center justify-center hidden px-4 transition-all cursor-pointer md:flex md:px-12 text-muted-foreground hover:text-white group-hover:opacity-100" onClick={() => scrollBy(title, -getScrollDistance())}>
							<ChevronLeft className="w-6 h-6 md:w-16 md:h-16" />
						</div>
					)}
					{canScrollRight[title] && (
						<div className="absolute inset-y-0 right-0 items-center justify-center hidden px-4 transition-all cursor-pointer md:flex md:px-12 text-muted-foreground hover:text-white group-hover:opacity-100" onClick={() => scrollBy(title, getScrollDistance())}>
