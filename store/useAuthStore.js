...
const fetchUserRolesClient = async (userId) => {
	try {
		...
		const { data: userProfile, error: profileError } = await supabase.from("users").select("role, business_id").eq("id", userId).maybeSingle();
		...
		// Add business_owner role if user owns a business
